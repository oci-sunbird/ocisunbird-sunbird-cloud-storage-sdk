...
class OCIS3StorageService(config: StorageConfig) extends BaseStorageService {
    ...
    // overrides.setProperty("jclouds.regions", "us-ashburn-1")
    ...
    override def upload(container: String, file: String, objectKey: String, isDirectory: Option[Boolean] = Option(false), attempt: Option[Int] = Option(1), retryCount: Option[Int] = None, ttl: Option[Int] = None): String = {
        try {
            if(isDirectory.get) {
                ...
                println(blobStore.blobBuilder(objectKey).payload(payload).getClass)
                val blobbuilder = blobStore.blobBuilder(objectKey).payload(payload).contentType(contentType).contentEncoding("UTF-8").contentLength(payload.size())
                println("**** Type of blob builder ****")
                println(blobbuilder.getClass)
